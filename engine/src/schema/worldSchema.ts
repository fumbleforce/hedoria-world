...
export const QuestSchema = z.object({
  ...
  /**
   * Authored design rationale — long-form prose used by editors and
   * occasionally surfaced in tooltips. Not consumed by the engine
   * runtime, so packs can omit it entirely.
   */
  questDesignBrief: z.string().optional(),
  detailType: z.enum(["basic", "detailed"]).default("basic"),
  questLocation: z.string().nullable().optional(),
  /**
   * NPC id of the quest-giver. Optional because some packs model
   * faction-wide objectives (e.g. League rank progressions) that don't
   * have a single giver. The dangling-ref check in `loader.ts`
   * gracefully no-ops when this is unset.
   */
  questGiverNPC: z.string().optional(),
  /**
   * Optional engine archetype that drives the quest tile populator. Quests
   * lacking this field still load and can be talked about; they just don't
   * spawn any QuestMarker overlays on the grids.
   */
  archetype: z
    .enum(["kill", "fetch", "clear", "escort", "delivery", "investigate"])
    .optional(),
  /**
   * Optional structured params consumed by the populator: e.g. for `kill`,
   * `{ monsterType: "boar", count: 5 }`; for `fetch`, `{ itemId, count }`;
   * for `delivery`, `{ targetLocation }`.
   */
  archetypeParams: z.record(z.string(), z.unknown()).optional(),
});
...
export type WorldNpc = z.infer<typeof NpcSchema>;
export type WorldNpcType = z.infer<typeof NpcTypeSchema>;
export type WorldLocation = z.infer<typeof LocationSchema>;
export type WorldRegion = z.infer<typeof RegionSchema>;
export type WorldQuest = z.infer<typeof QuestSchema>;
export type WorldItem = z.infer<typeof ItemSchema>;
